...
class WMDetectStableDiffusionPipeline(WatermarkStableDiffusionPipeline):
    ...
    def get_image_latents_ybb(self, image: torch.Tensor, sample=True, rng_generator=None, inference_mode=True):
        """
        获取图像的潜在变量表示
        :param image: 输入图像张量
        :param sample: 是否从分布中采样（True）或取均值（False）
        :param rng_generator: 随机数生成器
        :param inference_mode: 是否启用推理模式（禁用梯度计算）
        :return: 图像的潜在变量
        """
        # 根据参数选择上下文管理器
