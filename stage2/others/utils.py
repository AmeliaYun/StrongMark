...
def get_img_tensor(img_path, img_size, device):
    img_tensor = pil_to_tensor(Image.open(img_path).convert("RGB").resize((img_size, img_size)))/255  # (256, 256)
...
def watermark_prob(img, dect_pipe, wm_pipe, text_embeddings, tree_ring=True, device=torch.device('cuda'), logger=None):
    ...
    det_prob = wm_pipe.one_minus_p_value(reversed_latents) if not tree_ring else wm_pipe.tree_ring_p_value(reversed_latents, logger)
    return det_prob
