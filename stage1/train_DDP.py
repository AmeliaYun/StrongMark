...
def get_logger(filename, verbosity=1, name=None, rank=0):
    """只在主进程上创建logger"""
    if rank != 0:
        # 创建空logger给非主进程
...
parser.add_argument('--output_dir', type=str, default='results/Stage1_secret_size_120')
parser.add_argument('--image_size', type=int, default=256)
parser.add_argument('--num_steps', type=int, default=12000)
parser.add_argument('--warm_up_steps', type=int, default=0)
parser.add_argument('--batch_size', type=int, default=3)    # 每个gpu上的bs
...
parser.add_argument('--secret_size', type=int, default=120)      # --[debug] 48, 56, 96, 120
...
def train(rank, world_size):
    logger = get_logger(filename=f'{log_path}/training_enc_dec.log', name='Stage1_training_size256', rank=rank)

    setup(rank, world_size)
    torch.cuda.set_device(rank)
    device = torch.device('cuda')
    print(f"DEBUG - Rank {rank} started on GPU {torch.cuda.current_device()}")
    logger.info(f"Rank {rank} using device: {torch.cuda.get_device_name(rank)}")
    ...
    # 使用DDP包装模型
    sec_encoder = DDP(sec_encoder, device_ids=[rank])
    ...
    while global_step < args.num_steps:
        sec_encoder.train()
        decoder.train()
        train_sampler.set_epoch(global_step)  # 重要：确保每个epoch数据不同
        ...
        # 计算损失
        loss, secret_loss = model.build_model_ybb(secret_input, sec_encoder, decoder, image_input, loss_scales, args, global_step, vae, lpips_alex, logger, device, rank)
        ...
        if rank == 0:
            ...
            if global_step % args.validation_freq == 0:
                ...
                with torch.no_grad():
                    for batch in validation_dataloader:
                        ...
                        for distortion in distortion_list:
                            avg_psnr_input, avg_psnr_recons, predict_acc_WM = model.validate_model(secret_input, sec_encoder, decoder, image_input, vae, distortion, args.image_size)
                ...
                # 计算平均值
                avg_acc_WM = torch.tensor(acc_WM_ls).mean().item()
                avg_psnr_input = torch.tensor(psnr_input_ls).mean().item()
                avg_psnr_recons = torch.tensor(psnr_recons_ls).mean().item()
